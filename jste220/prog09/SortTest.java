...
public class SortTest<E extends Comparable<E>> {
  ...
  public boolean sameContents (E[] array1, E[] array2) {
    // EXERCISE
	  Map<E, Integer> first = new HashMap<E, Integer>();
	  Map<E, Integer> second = new HashMap<E, Integer>();
    ...
    // same integer in both maps.  If not, return false.
	  for (int num = 0; num < array1.length; num++) {
		  if (first.get(array1[num]) != second.get(array1[num])) {
			  System.out.println(array1[num] + "does not equal" + array2[num]);
			  return false;
		  }
	  }
